...
const averageSide = (
  ...
  const ready = players
    ...
    .filter(
      (state): state is Extract<RecentStatsState, { status: "ready" }> => state?.status === "ready",
    );
  if (ready.length === 0)
    return { teamId, players: 0, kd: null, winRate: null, acs: null, dpr: null };
...
export const buildTeamMatchup = (
  ...
  return {
    ally: averageSide(
      self.teamId,
      players.filter((player) => player.teamId === self.teamId),
      recent,
    ),
    enemy: averageSide(
      enemy.teamId,
      players.filter((player) => player.teamId === enemy.teamId),
      recent,
    ),
  };
};
