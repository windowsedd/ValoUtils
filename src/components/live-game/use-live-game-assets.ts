...
export const useLiveGameAssets = (players: LivePlayer[]): LiveGameAssets => {
  ...
  useEffect(() => {
    ...
    for (const player of players) {
      if (player.loadout)
        weapons.push(player.loadout.vandal, player.loadout.phantom, player.loadout.knife);
    }
    const uniqueWeapons = [
      ...new Map(weapons.filter(Boolean).map((weapon) => [weaponSkinKey(weapon), weapon])).values(),
    ];
    pooled(
      uniqueWeapons.map(
        (weapon) => () =>
    ...
    const cardIds = [
      ...new Set(players.map((player) => player.cardId).filter(Boolean) as string[]),
    ];
    pooled(
      cardIds.map(
        (id) => () => getPlayerCard(id).then((asset) => [id.toLowerCase(), asset] as const),
      ),
    ).then((entries) => {
      if (cancelled) return;
      setAssets((current) => {
