...
export const navbarLayout = {
  ...
  railRoutes:
    "command-rail-scroll flex min-h-0 w-full flex-1 flex-col items-center gap-2 overflow-y-auto overflow-x-hidden",
  railBottom: "flex shrink-0 flex-col items-center pt-3",
  railStatus: "mt-3 shrink-0",
  railButton:
    "press-tile group navbar-motion relative grid h-10 w-10 shrink-0 place-items-center rounded-[10px] border border-transparent text-[18px] outline-none transition-[color,background-color,border-color,box-shadow,scale] duration-150 focus-visible:border-(--accent) focus-visible:shadow-[0_0_0_2px_var(--accent-soft)]",
  railButtonActive:
    "bg-(--accent) text-(--accent-foreground) shadow-[0_2px_10px_rgba(128,100,233,0.32)]",
  ...
  statusTriggerCompact:
    "press-flat group navbar-motion relative grid h-10 w-10 place-items-center rounded-[10px] border border-transparent text-(--text-muted) outline-none transition-[color,background-color,border-color,box-shadow] duration-150 hover:border-(--border) hover:bg-(--surface-hover) hover:text-(--text-primary) focus-visible:border-(--accent) focus-visible:shadow-[0_0_0_2px_var(--accent-soft)]",
  statusMenu:
    "absolute right-0 top-11 z-50 w-60 max-w-[calc(100vw-1rem)] rounded-[8px] border border-(--border) bg-(--surface) p-1 shadow-[0_8px_24px_rgba(0,0,0,0.28)]",
  statusMenuCompact:
    "absolute bottom-0 left-full z-50 ml-3 w-60 max-w-[calc(100vw-5rem)] rounded-[8px] border border-(--border) bg-(--surface) p-1 shadow-[0_8px_24px_rgba(0,0,0,0.28)]",
  statusMessage:
    "mt-1 whitespace-normal break-words border-t border-(--line) px-2.5 pt-2 text-[11px] leading-4",
} as const;
