	...
	return (
		<div className={`flex min-h-0 flex-col ${embedded ? "" : "h-full animate-fade-in"}`}>
			{!embedded && (
				<PageHeader
					icon={
						<button type="button" onClick={onBack} title={t("friends.profileBack")} aria-label={t("friends.profileBack")} className="grid h-10 w-10 place-items-center rounded-md border border-white/10 text-gray-400 hover:bg-white/6 hover:text-white focus-visible:outline-none focus-visible:shadow-[0_0_0_2px_var(--accent-soft)]">
							<FaArrowLeft />
						</button>
					}
					title={displayName}
