...
export const normalizeTranslationCode = (
  ...
  if (role === "source" && typeof value === "string" && value.trim().toLowerCase() === "auto") {
    return "auto";
  }
  if (typeof value !== "string") return null;
  const normalized = value.trim().toLowerCase();
  return (
    getTranslationLanguages(provider, role).find((item) => item.code.toLowerCase() === normalized)
      ?.code ?? null
  );
};
...
export const displayTranslationLanguage = (language: TranslationLanguage, locale: string) => {
  try {
    const resolved = new Intl.DisplayNames([locale], { type: "language" }).of(language.code);
