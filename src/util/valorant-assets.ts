...
const accessoryFromJson = (item: { displayName?: Localized; titleText?: Localized; fullTransparentIcon?: string; displayIcon?: string; largeArt?: string } | null): SkinAsset | null => {
	if (!item) return null;
...
const collectPassRewardIds = (contracts: readonly unknown[]): Set<string> => {
	...
	for (const contract of contracts) {
		...
		const content = (contract as { content?: { relationType?: unknown; chapters?: unknown } }).content;
		...
		inventoryIndexPromise = Promise.all([
			...
			.then(([weapons, sprays, buddies, cards, titles, flexes, themesJson, contracts]) => {
				...
				for (const theme of themesJson?.data ?? []) {
					if (typeof theme?.uuid === "string") themeNames.set(theme.uuid.toLowerCase(), theme.displayName);
				}
				...
				for (const weapon of weapons?.data ?? []) {
					...
					for (const skin of weapon?.skins ?? []) {
						...
						const tierId = typeof skin.contentTierUuid === "string" ? skin.contentTierUuid.toLowerCase() : "";
						const tierName = TIER_NAME_BY_ID[tierId] ?? null;
						const themeId = typeof skin.themeUuid === "string" ? skin.themeUuid.toLowerCase() : null;
						const levelIds = (skin.levels ?? [])
							.map((level: { uuid?: string }) => (typeof level?.uuid === "string" ? level.uuid.toLowerCase() : ""))
							.filter(Boolean);
				...
				for (const spray of sprays?.data ?? []) {
					...
					for (const level of spray.levels ?? []) {
						...
						putAccessory("sprays", level.uuid, parent, spray.displayName, level.displayIcon || icon);
					}
				}
				for (const buddy of buddies?.data ?? []) {
					...
					for (const level of buddy.levels ?? []) {
						...
						putAccessory("buddies", level.uuid, parent, buddy.displayName, level.displayIcon || icon);
					}
				}
				for (const card of cards?.data ?? []) {
					const icon = card.smallArt || card.displayIcon || "";
					putAsset(assets, card.uuid, card.displayName, icon);
					putAccessory("cards", card.uuid, typeof card.uuid === "string" ? card.uuid.toLowerCase() : "", card.displayName, icon);
				}
 ...
 */
export const getStoreItem = async (itemTypeId: string | undefined, itemId: string): Promise<SkinAsset | null> => {
