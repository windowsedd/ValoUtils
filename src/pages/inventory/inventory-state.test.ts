...
describe("inventory filters and spend", () => {
	...
	test("search matches localized names and falls back to the raw id", () => {
		expect(filterInventory(items, "all", "vandal", names).map((row) => row.itemId)).toEqual(["skin-1"]);
		expect(filterInventory(items, "all", "TITLE-1", names).map((row) => row.itemId)).toEqual(["title-1"]);
