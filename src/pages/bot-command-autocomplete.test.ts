...
describe("bot command template autocomplete", () => {
	...
	test("maps navigation, commit, and dismissal keys", () => {
		expect(autocompleteKeyAction("ArrowDown", 0, 3)).toEqual({ handled: true, activeIndex: 1, commit: false, dismiss: false });
		expect(autocompleteKeyAction("ArrowUp", 0, 3)).toEqual({ handled: true, activeIndex: 2, commit: false, dismiss: false });
