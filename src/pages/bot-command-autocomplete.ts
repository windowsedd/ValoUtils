...
export const findActiveTemplateRange = (
  value: string,
  caret: number,
): ActiveTemplateRange | null => {
...
export const filterTemplateVariables = (
  ...
  return BOT_TEMPLATE_VARIABLES.filter(
    (item) => item.id.includes(wanted) || descriptionOf(item).toLocaleLowerCase().includes(wanted),
  );
};
...
export const autocompleteKeyAction = (key: AutocompleteKey, activeIndex: number, count: number) => {
  ...
  if (key === "Enter" || key === "Tab")
    return { handled: count > 0, activeIndex, commit: count > 0, dismiss: false };
  if (key === "ArrowDown" && count > 0)
    return { handled: true, activeIndex: (activeIndex + 1) % count, commit: false, dismiss: false };
