...
export const parseReward = (value: {
  ...
  const amount =
    typeof value.amount === "number" && Number.isFinite(value.amount) ? value.amount : 1;
  return {
    type,
    uuid,
    amount,
  ...
  for (const contract of data) {
    ...
    const chapters = Array.isArray(row.content?.chapters)
      ? row.content.chapters.map((chapter) =>
          parseChapter(
            (chapter && typeof chapter === "object" ? chapter : {}) as Parameters<
              typeof parseChapter
            >[0],
          ),
        )
...
export const pageIndexForLevel = (
