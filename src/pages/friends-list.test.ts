...
describe("Friends visible roster", () => {
	test("hides only ValoUtils Bot#BOT", async () => {
		...
		expect(
			isVisibleFriend({ gameName: "ValoUtils Bot", tagLine: "BOT" }),
		).toBe(false);
		expect(
			isVisibleFriend({ gameName: " valoutils bot ", tagLine: " bot " }),
		).toBe(false);
		expect(
			isVisibleFriend({ gameName: "ValoUtils Bot", tagLine: "PLAYER" }),
		).toBe(true);
		expect(isVisibleFriend({ gameName: "Another", tagLine: "BOT" })).toBe(
			true,
		);
	});
});
