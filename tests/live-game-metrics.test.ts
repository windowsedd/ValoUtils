...
const ready = (kd: number, winRate: number, acs: number): RecentStatsState => ({
	status: "ready",
	stats: { matches: 5, kills: 0, deaths: 0, assists: 0, wins: 0, kd, winRate, acs, history: [] },
});
...
describe("buildTeamMatchup", () => {
	test("averages ready players on the signed-in and opposing teams", () => {
		...
		const recent = { p1: ready(1, 50, 200), p2: ready(2, 60, 220), p3: ready(0.8, 40, 180) };
	...
	test("keeps metrics empty while one team has no ready statistics", () => {
		...
		expect(buildTeamMatchup(players, { p1: ready(1.2, 60, 230), p2: { status: "loading" } })).toEqual({
			ally: { teamId: "Ally", players: 1, kd: 1.2, winRate: 60, acs: 230 },
			enemy: { teamId: "Enemy", players: 0, kd: null, winRate: null, acs: null },
		});
	});
