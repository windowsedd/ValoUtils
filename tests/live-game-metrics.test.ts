...
const player = (puuid: string, teamId: string, isSelf = false) =>
...
describe("buildTeamMatchup", () => {
  test("averages ready players on the signed-in and opposing teams", () => {
    ...
    const recent = {
      p1: ready(1, 50, 200, 140),
      p2: ready(2, 60, 220, 160),
      p3: ready(0.8, 40, 180, 120),
    };
  ...
  test("keeps metrics empty while one team has no ready statistics", () => {
    ...
    expect(
      buildTeamMatchup(players, { p1: ready(1.2, 60, 230, 155), p2: { status: "loading" } }),
    ).toEqual({
