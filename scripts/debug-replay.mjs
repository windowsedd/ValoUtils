		...
		...(options.includeDetails
			? {
					details: {
						...
						movementSamplesPerSecond: result.replay.Info.LengthInMs > 0 ? +(result.movement.length / (result.replay.Info.LengthInMs / 1000)).toFixed(2) : 0,
...
async function runMain() {
	...
	const timer = quiet ? null : setInterval(() => renderProgress(startedAt, filePath, fileSize, phase), process.stderr.isTTY ? 250 : 5000);
